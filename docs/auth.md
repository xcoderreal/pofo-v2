...
- **JWT verification:** ES256, against the project's public signing key fetched from its JWKS endpoint (`{MYAPP_SUPABASE_URL}/auth/v1/.well-known/jwks.json`), via `pyjwt[crypto]`'s `PyJWKClient` — algorithm pinned explicitly (never `alg: none`), `aud`/`exp` validated. No shared secret is configured or needed: Supabase signs session tokens asymmetrically by default (a per-project EC keypair, `kid`-matched against the published JWKS), for both local CLI stacks and hosted projects — this isn't a local-dev quirk or an opt-in upgrade path, it's the current default. An earlier version of this doc claimed HS256 with a shared secret was the default; that was wrong — confirmed by testing against a real (local) Supabase Auth server and hitting `{"detail":"The specified alg value is not allowed"}`, not by reading changelog. `PyJWKClient` caches the fetched key, keyed by `kid`, so this isn't a fetch-per-request cost. `SupabaseAuthProvider`'s constructor takes an injectable `get_signing_key` callable specifically so tests can supply a known test key instead of hitting a real JWKS endpoint over the network.
